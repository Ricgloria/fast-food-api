...
router.get('/', userToken, (req, res, next) => {
    ...
    mysql.getConnection((err, conn) => {
        ...
        conn.query(
            `SELECT sa.id_sale, sa.sale_date, sa.sale_value, u.name, pm.description
             FROM sales sa
                      INNER JOIN users u on u.id_user = sa.id_employee
                      INNER JOIN payment_methods pm on sa.id_payment_methods = pm.id_payment_method
...
router.post('/', userToken, (req, res, next) => {
    ...
    mysql.getConnection((err, conn) => {
        ...
        const query = `INSERT INTO sales (sale_date, sale_value, id_employee, id_payment_methods)
                       VALUES (?, ?, ?, ?)`;
